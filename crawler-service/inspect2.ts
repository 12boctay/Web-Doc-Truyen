...
async function inspect() {
  ...
  await page.goto('https://truyenqqno.com/tim-kiem/trang-1.html?q=one+punch+man', {
  ...
  if (searchResults.length > 0) {
    const detailUrl =
      searchResults.find((r) => r.text.toLowerCase().includes('one punch'))?.href ||
    ...
    const detail = await page.evaluate(() => {
      const h1s = Array.from(document.querySelectorAll('h1')).map((e) => ({
        class: e.className,
      ...
      const detailContainers = Array.from(
        document.querySelectorAll(
          '[class*="detail"], [class*="book_detail"], [class*="comic"], [class*="manga"], [class*="story"]',
        ),
      ).map((e) => ({
        tag: e.tagName,
      ...
      const infoItems = Array.from(
        document.querySelectorAll('[class*="info"] li, [class*="info"] p, [class*="detail"] li'),
      ).map((e) => ({
      ...
      // Chapter list
      const chapters = Array.from(document.querySelectorAll('a[href*="chap"]'))
        .slice(0, 5)
        .map((a) => ({
          href: a.getAttribute('href'),
          text: a.textContent?.trim()?.substring(0, 80) || '',
      ...
      const mainContent = document.querySelector(
        '[class*="book_detail"], [class*="detail_story"], [class*="story-detail"], main, .main_content, #main_content',
      );
      const mainHtml =
        mainContent?.innerHTML?.substring(0, 3000) || document.body.innerHTML.substring(0, 5000);
