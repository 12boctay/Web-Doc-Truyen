...
async function inspect() {
  ...
  const detail = await page.evaluate(() => {
    ...
    for (const sel of trySelectors) {
      ...
      if (el) {
        ...
        if ((el as HTMLImageElement).src)
          attrs.push(`src="${(el as HTMLImageElement).src.substring(0, 100)}"`);
        if ((el as HTMLAnchorElement).href)
          attrs.push(`href="${(el as HTMLAnchorElement).href.substring(0, 100)}"`);
        selectorTests[sel] =
          `[${tag}] ${el.textContent?.trim()?.substring(0, 120) || '(empty)'} ${attrs.join(' ')}`;
      }
    }
    ...
    const relevantEls = $$(
      '[class*="book"], [class*="detail"], [class*="chapter"], [class*="story"], [class*="info"], [class*="list"]',
    );
    ...
    const mainEl =
      ...
      $('[class*="detail"]');
    const mainHtml =
      mainEl?.outerHTML?.substring(0, 6000) || document.body.innerHTML.substring(1000, 7000);

    // Chapter links cụ thể
    const chapterLinks = $$('a[href*="chap"]')
      .slice(0, 5)
      .map((a) => ({
  ...
  if (chapterUrl) {
    const fullChapterUrl = chapterUrl.startsWith('http')
      ? chapterUrl
      : `https://${workingDomain}${chapterUrl}`;
    ...
    const chapter = await page.evaluate(() => {
      ...
      for (const sel of containers) {
        ...
        if (el)
          foundContainers[sel] =
            `children: ${el.children.length}, imgs: ${el.querySelectorAll('img').length}`;
      }
      ...
      const mainHtml =
        (
          ...
          $('#chapter_content') ||
          $('[class*="chapter"]')
        )?.innerHTML?.substring(0, 3000) || 'NOT FOUND';
