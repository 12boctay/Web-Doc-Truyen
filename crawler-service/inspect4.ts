...
async function inspect() {
  ...
  const data = await page.evaluate(() => {
    ...
    const selectors = [
      ...
      '.manga-info', '.comic-info', '.book_other', '.book_name',
      '.list_chapter', '.list-chapter', '.works-chapter-list',
      '.book_avatar', '.book_desc', '.story-detail-info',
    ];
    ...
    const relevantEls = els('[class*="book"], [class*="detail"], [class*="chapter"], [class*="story"], [class*="list_chapter"]');
    ...
    const mainArea = el('.main_content') || el('#main_content') || el('.content') || el('.container') || el('.div_middle');
    const mainHtml = mainArea ? mainArea.innerHTML.substring(0, 8000) : document.body.innerHTML.substring(2000, 10000);
