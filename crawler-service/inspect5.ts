...
async function inspect() {
  ...
  await page.goto('https://truyenqqno.com/truyen-tranh/dai-quan-gia-la-ma-hoang-7015', { waitUntil: 'domcontentloaded', timeout: 30000 });
  ...
  const found = await page.evaluate(() => {
    var selectors = [
      '.book_detail', '.book_info', '.detail-info', '.story-detail',
      '.book_other', '.book_name', '.list_chapter', '.list-chapter',
      '.works-chapter-list', '.book_avatar', '.book_desc', '.story-detail-info',
    ];
    var result = {};
  ...
  const classes = await page.evaluate(() => {
    var els = document.querySelectorAll('[class*="book"], [class*="detail"], [class*="chapter"], [class*="story"]');
    var result = [];
  ...
  const html = await page.evaluate(() => {
    var main = document.querySelector('.book_detail') || document.querySelector('.detail') || document.querySelector('main');
  ...
  const chapterHtml = await page.evaluate(() => {
    var chList = document.querySelector('.list_chapter') || document.querySelector('.list-chapter') || document.querySelector('[class*="chapter"]');
