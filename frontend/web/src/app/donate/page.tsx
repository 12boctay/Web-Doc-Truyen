...
import { useActiveGoals, useRecentDonations, useTopDonors, useCreateDonation } from '@/hooks/useDonate';
        ...
        <div className="md:col-span-2">
                  ...
                  <div key={goal._id} className="rounded-xl border border-gray-200 bg-white p-4">
                    ...
                    <div className="mt-3">
                      <div className="mb-1 flex justify-between text-sm">
                        <span className="font-medium text-green-600">{formatMoney(goal.currentAmount)}</span>
          ...
          {!isAuthenticated ? (
            <div className="rounded-xl border border-gray-200 bg-white p-6 text-center">
              ...
              <Link href="/dang-nhap" className="rounded-lg bg-blue-600 px-6 py-2 text-sm font-medium text-white hover:bg-blue-700">
                Đăng nhập
              </Link>
            </div>
          ) : success ? (
            <div className="rounded-xl border border-green-200 bg-green-50 p-6 text-center">
              ...
              {method === 'bank_transfer' && (
                <div className="mx-auto mt-4 max-w-sm rounded-lg border border-green-200 bg-white p-4 text-left text-sm">
                  ...
                  <p><strong>Nội dung CK:</strong> DONATE_{user?._id?.slice(-6)}</p>
                  <p><strong>Số tiền:</strong> {formatMoney(finalAmount)}</p>
                </div>
              )}
              <button
        ...
        <div className="space-y-6">
                  ...
                  <div key={d._id} className="flex items-center justify-between text-sm">
                    <div>
                      ...
                      {d.userId?.donorBadge && d.userId.donorBadge !== 'none' && (
                        <span className={`ml-1 text-xs font-medium ${BADGE_STYLES[d.userId.donorBadge]?.color || ''}`}>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-gray-400">{timeAgo(d.completedAt || d.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {topDonors.map((d: any, i: number) => (
                  <div key={d._id} className="flex items-center gap-2 text-sm">
                    ...
                    {d.donorBadge && d.donorBadge !== 'none' && (
                      <span className={`text-xs font-medium ${BADGE_STYLES[d.donorBadge]?.color || ''}`}>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4">
            ...
            <div className="space-y-1.5 text-xs">
              <p><span className="text-cyan-500 font-medium">Diamond</span> — từ 2,000,000đ</p>
              <p><span className="text-yellow-500 font-medium">Gold</span> — từ 1,000,000đ</p>
              <p><span className="text-gray-400 font-medium">Silver</span> — từ 500,000đ</p>
              <p><span className="text-orange-600 font-medium">Bronze</span> — từ 100,000đ</p>
            </div>
          </div>
        </div>
      </div>
